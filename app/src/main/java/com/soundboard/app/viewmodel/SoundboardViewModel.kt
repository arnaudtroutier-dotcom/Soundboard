...
class SoundboardViewModel(application: Application) : AndroidViewModel(application) {
    ...
    viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
        val fresh = tileDao.getTileById(tile.id) ?: return@launch
        tileDao.update(fresh.copy(posX = posX.coerceIn(0f, 1f), posY = posY.coerceIn(0f, 1f)))
    }
}
